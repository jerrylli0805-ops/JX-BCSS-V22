...
// [MODIFIED] 安全渲染圖示 (除錯 + 強制顯色版)
function safeRenderIcons() {
    // 1. 抓取所有帶有 data-lucide 的標籤 (不限於 i 標籤，防止 selector 漏抓)
    const elements = document.querySelectorAll('[data-lucide]');
    
    // (除錯用) 如果找到元素，在 console 顯示數量
    if (elements.length > 0) {
        console.log(`[Icon System] Found ${elements.length} icons to render.`);
    }
    ...
    elements.forEach(element => {
        ...
        if (OFFLINE_ICONS && OFFLINE_ICONS[iconName]) {
            ...
            // [關鍵修改] 優先使用 currentColor，但如果 CSS 沒載入，強制給一個深色備案
            svg.setAttribute('stroke', 'currentColor'); 
            svg.style.stroke = element.style.color || 'currentColor'; 
            ...
            // [關鍵修改] 如果 Tailwind 斷網導致 w-4 h-4 失效，這裡強制補上 inline style 大小
            if (!originalClass.includes('w-') || !originalClass.includes('h-')) {
            ...
            // 如果找不到圖示，印出錯誤以便除錯
            console.warn(`[Icon System] Missing icon definition for: ${iconName}`);
            // 可以在畫面上顯示一個問號替代，證明程式有跑
            element.textContent = '(?)';
        }
    });
}
